...
if __name__ == '__main__':
    # be careful with the spacing when naming it
    example_board = [
        [3, 9, -1,   -1, 5, -1,   -1, -1, -1],
        [-1, -1, -1,   2, -1, -1,   -1, -1, 5],
        [-1, -1, -1,   7, 1, 9,   -1, 8, -1],
...
# I tried to make a graphical user surface. Some features work, but some others won't. I will keep it updated.
def solve():
    try:
        puzzle = [[int(entries[i][j].get()) for j in range(9)] for i in range(9)]
        for row in puzzle:
            for num in row:
                if not(-1 <= num <= 9):
                    raise ValueError("Number out of range")
    except ValueError:
        tk.messagebox.showerror("Invalid input", "Please enter only numbers from -1 to 9")
        return
    solve_sudoku(puzzle)
    for i in range(9):
        for j in range(9):
...
# create a grid of text entry fields
entries = [[tk.Entry(root, width=2) for j in range(9)] for i in range(9)]
for i in range(9):
    for j in range(9):
        entries[i][j].grid(row=i, column=j)
        entries[i][j].insert(0, str(example_board[i][j]))  # insert values from example_board

# create a button that will solve the puzzle when clicked
